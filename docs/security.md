...
有兩個、而且只有兩個對外連線，都是使用者自己打開的：自動抓收盤價，和 AI 健檢。兩個守的是
同一組線，讓「離線」仍然是預設而不是被推翻：預設關、在後端連、網址釘在一個檔案裡、送出去的
東西說清楚。

### 自動抓收盤價

使用者在設定裡打開後，每天啟動服務時會到 Yahoo 抓每一檔持股的最新收盤價（收盤後更新最準）：
...
送出的東西最多：資產全覽整份（帳戶名稱、餘額、持股、近一年的消費分類），交給使用者自己選的
Claude、OpenAI 或 Gemini，用使用者自己的 key。所以它比抓價多守了幾條：

- **預設關，打開了也只在按下「送出」時送。** 沒有背景工作、沒有排程，啟動時不送；一次按下就是
  一次請求。
- **在後端送**（`server/ai.js`，`node:https`），瀏覽器那頁的 CSP 一樣不變。
- **三個端點都釘在 `server/ai.js`**，每個都是整段網址比對：`api.anthropic.com/v1/messages`、
  `api.openai.com/v1/responses`、`generativelanguage.googleapis.com/v1beta/models/`。
- **送出前先給你看，送的就是看過的那份**：頁面上可以展開完整的說明和文件（跟「匯出全覽」下載的
  檔一個位元組都不差）。送出時頁面送回預覽的 digest，後端從帳本重新產生、對不上就 409，不收頁面
  給的文字——看過之後帳本或設定變了，就一個字都不送。
- **key 不在帳本裡**：存在帳本旁邊的 `ai-keys.json`（權限 `600`），或用那家平常的環境變數；
  API 只回「有沒有設定」和最後四碼。快照、備份、JSON 匯出裡都沒有它，`.gitignore` 和
  pre-commit hook 也都擋它。
- **回答一律當文字**：模型寫的 HTML 照字面顯示，連結不會變成可以點的連結。
- 示範版沒有伺服器，CSP 又是 `connect-src 'none'`，所以**沒有能力**用；它的 API 對每一條 AI
  路由都回「不能用」。

細節（每家的資料政策、模型怎麼選、失敗時看到什麼）在 [`ai.md`](ai.md)。
...
`test/deps.test.js` 的不對外連線掃描放行那個網址，而且是完整比對而不是網域前綴——前綴會連
`<script src>` 一起放行。它放行的理由跟上面兩個例外不一樣：那不是請求，是一個人自己點的
`<a href>`，`default-src 'none'` 一樣不准從那個網域載入任何東西。
