...
資料存在 `~/.finance-hub/finance.db`，**不上傳任何雲端、預設不對外連線、不需要帳號密碼**。
伺服器只綁 `127.0.0.1`。
...
```bash
FINANCE_PROFILE=demo node scripts/seed-demo.js   # 三年、十個帳戶的編造資料
...
才有持股歷史，淨值走勢才能含持股，也才能算 XIRR 和時間加權報酬。目前股價是手動填的，或打開
預設關閉的收盤價抓取。
